...
function loadData(d,val){
  ...
  .on("mouseover",function(inp){
    ...
    reLoadDataWithNewBins(d,val,this.value);
}
  ...
  function reLoadDataWithNewBins(d,val,slider){
    console.log(typeof(+slider)+"GG");
    //console.log(typeof(+value);
    loop=0;//looping through any array in the code
    numBins = +slider;//number of bins/bars in the graph
    binSize = 0;//range of values bin can hold i.e binSize = 5 if it can hold 290-294 values
    xRanges = new Array(numBins).fill(0);//initialize an array with 0. This array is to define the range of values x Axis will be taking
    binArray = new Array(numBins).fill(0);//initialize an array with 0. This array is to define the count of values in each bin
    maxValue = 0;//min Value in the input
    minValue = 0;//max value in the input
    binWidth = width/(numBins);//calculate the width of the rect aka bar
    columnNames = [0]; //populate all the columnNames
    dropDownValue = "gre";
    chartSwitch = "bar";
    ...
    console.log("Min Value"+min+" "+factor)
    console.log("Max Value"+xRanges.length)
    for(loop=0;loop<xRanges.length;loop++)
    {
      xRanges[loop] = Math.floor((min+(min+factor))/2);
      min = min + factor;
